-- ============================================================
-- PicMyEvent · SCRIPT UNIVERSAL IDEMPOTENTE (0001-0012 + Realtime)
-- Cada create policy lleva su drop policy if exists previo:
-- se puede ejecutar VARIAS VECES y en cualquier estado del esquema.
-- ============================================================

-- --- Dropear el tipo events_public SOLO si existe (CASCADE libera
-- --- get_event_public, que se recrea con CREATE OR REPLACE) ---
do $$
begin
  if exists (select 1 from pg_type where typname = 'events_public' and typnamespace = 'public'::regnamespace) then
    execute 'drop type public.events_public cascade';
  end if;
end $$;
...
drop policy if exists "event_members_select_member" on public.event_members;
create policy "event_members_select_member"
  on public.event_members for select
  using (public.is_event_member(event_id, auth.uid()));

drop policy if exists "event_members_insert_member" on public.event_members;
...
drop policy if exists "event_members_delete_member" on public.event_members;
...
drop policy if exists "drive_connections_select_owner" on public.drive_connections;
create policy "drive_connections_select_owner"
  on public.drive_connections for select
  using (organizer_id = auth.uid());

drop policy if exists "drive_connections_update_owner" on public.drive_connections;
create policy "drive_connections_update_owner"
  on public.drive_connections for update
  using (organizer_id = auth.uid())
  with check (organizer_id = auth.uid());

drop policy if exists "drive_connections_delete_owner" on public.drive_connections;
...
-- --- Realtime: publicar photos, likes, comments (solo si faltan) ---
do $$
begin
...
-- Verificacion: select tablename from pg_tables where schemaname='public' order by tablename;
